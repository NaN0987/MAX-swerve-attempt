...
public class AutoAlignCircle extends CommandBase {
    ...
    public void execute() {
        ...
        if (targets > 0) {
            ...
            // Calculate desired position in circular orbit
            double desiredX = orbitRadius * Math.cos(angleToTarget);
            double desiredY = orbitRadius * Math.sin(angleToTarget);

            // Calculate proportional control errors
            double errorX = desiredX - x;
            double errorY = desiredY - y;

            // Adjust crabCrawl and forwardSpeed based on proportional control
            double crabCrawl = orbitSpeed * errorX;
